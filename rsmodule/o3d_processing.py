...
def combine_point_clouds(
    ...
    if len(updated_global_map_pcd.points) > target_num_points:
        every_k_points = max(1, len(updated_global_map_pcd.points) // target_num_points)
        updated_global_map_pcd = updated_global_map_pcd.uniform_down_sample(every_k_points=every_k_points)
